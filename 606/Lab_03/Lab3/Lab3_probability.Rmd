...
**Streak length of 1 means there was one hit. There can be one miss in a streak of one, but a streak of one means one hit. Streak of 0 means miss.**
...
```{r calc-streak-kobe}
kobe_streak <- tibble(streak = as.integer(unlist(calc_streak(kobe_basket$shot))))
kobe_streak |> ggplot(aes(x = streak)) + geom_bar()
```
...
**Kobe had more streak lengths of 0 than any other. Typically it was either a streak of 0 or 1. His longest streak was 4.**
```{r}
kobe_streak |> ggplot(aes(x = streak)) + geom_bar()
```
...
set.seed(13)
coin <- c("heads", "tails")
sim_coin <- sample(coin_outcomes, size = 100, replace = TRUE, prob = c(0.2, 0.8))
table(sim_coin)
```

**20/100 flips were heads. **
...
```{r set-seed}
set.seed(13)                 # make sure to change the seed
```
...
table(sim_basket)
```
**You change the probability of H to be 0.45.**
...
```{r}
sim_streak <- as.integer(unlist(calc_streak(sim_basket)))
table(sim_streak)
```
**Answer in code above.**
...
```{r}
sim_df <- tibble(streak = sim_streak)
sim_df |> ggplot(aes(x = streak)) + geom_bar()
```

**Typical streak length is 0-1, the longest being 8!**
...
**It would be the same since the seed is set to 13. No matter how many times I run it, the sample will be the same unless I remove the set.seed() function or set.seed(NULL)**
...
```{r}
df <- bind_rows(kobe = kobe_streak, sim = sim_df, .id = "player") |> unnest(cols = everything())
df |> ggplot(aes(x = streak, fill = player)) + geom_bar()
df |> count(player, streak, name = "count") |> pivot_wider(names_from = streak, values_from = count, names_prefix = "streak_") 
```
**It's kinda different. From the table we can see that Sim (simulation) had more streaks of 0 than Kobe, but Sim also had a single streak of 5 and 8, which Kobe never was able to get. They also each had a tie for streaks of 4. I'd say that Sim performed better than Kobe.**
