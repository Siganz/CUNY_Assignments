...
```{r r-interface-2020, eval=FALSE, include=FALSE, out.width='100%', results="asis"}
...
This file (with the `.Rmd` file extension) will serve as the lab report.
You can just type your answers in this document instead of creating a separate document.
...
R has added numbers in [brackets] along the left side of the printout to indicate locations within the vector.
...
**There is a positive trend with year and girls, particularly among the later years. We can view this from the line generated by geom_smooth() in the ggplot above, but we can also just double check by finding the correlation between girls and years:**

```{r}
cor(arbuthnot$year, arbuthnot$girls)
```

**Cor is approximately 0.8, which is a strong positive correlation between year and girls.**
...
We could repeat this once for each year, but there is a faster way.
...
It takes the output of the previous expression and pipes it into the first argument of the function in the following one.
...
**Observation: The greater the year, the less the ratio of boys to total there are being baptized.**
...
**Or we can read the glimpse()**

```{r}
glimpse(present)
```

**The data above shows the following: Rows = 63 | Cols: 3 | Columns = year, boys, girls.**
------------------------------------------------------------------------
...
print("Arbuthnot df has more rows, but each total value is magnitude's less than the present df.")

a_sum <- sum(arbuthnot$total)
p_sum <- sum(present$total)

sprintf("arbuthnot total count = %d",a_sum)
sprintf("present total count = %d",p_sum)
sprintf("arbuthnot's total count is %.2f percent of present's total count",((a_sum/p_sum)*100))

arbuthnot |>
  arrange(desc(boys)) |>
  slice_max(boys, na_rm = FALSE)
present |>
  arrange(desc(boys)) |>
  slice_max(boys, na_rm = FALSE)
```
**Arbthnot's total data is between thousands and tens of thousands, while Present's total data is in the millions -- They are different in magnitude.**
...
**I see that the proportion of boys from the total decreases as the years increase.**
...
**If we take a look at the earlier arbuthnot boy_ratio, we can see the same thing. We can see the correlation using cor()**

```{r}
p_cor <- cor(present$year, present$boy_ratio)
a_cor <- cor(arbuthnot$year, arbuthnot$boy_ratio)
sprintf("Present cor between year and boy_ratio: %f",p_cor)
sprintf("Arbuthnot cor between year and boy_ratio: %f",a_cor)
sprintf("The difference is: %f",abs(p_cor - a_cor))
```

**Both datasets show a negative correlation between boy_ratio and year. The present df shows a stronger negative correlation than the arbuthnot df.**
...
glimpse(present)
present |>
  slice_max(total, n = 1, na_rm = TRUE)
```

**1961 is the year with the highest total number of births, based on the present df.**
...
If you are googling for R code, make sureto also include these package names in your search query.
For example, instead of googling "scatterplot in R", google "scatterplot in R with the tidyverse".
