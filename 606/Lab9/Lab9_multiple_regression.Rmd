...
**It's most certainly an observational study and we cannot say for certain whether or not beauty leads directly to differences in course evals, but we can make an association.**

2.  Describe the distribution of `score`. Is the distribution skewed? What does 
    that tell you about how students rate courses? Is this what you expected to 
    see? Why, or why not?

**Score is left skewed, unimodal, and most scores range between 4 and 5 with the center around 4.5. I think this is expected as most students try to be nice with their professors and rate the class highly, but doubt they would give it a perfect 5 since "there's always room for improvement". So it's expected.**

```{r}
evals |> select(score) |> ggplot(aes(x=score)) + geom_histogram(bins = 15)
```

3.  Excluding `score`, select two other variables and describe their relationship 
    with each other using an appropriate visualization.

**Haha, didn't expect that males would get a higher median score than female professors. **
```{r}
evals |> select(gender, score) |> ggplot(aes(x=gender, y = score, fill = gender)) + geom_boxplot()
...
**The initial scatterplot showed a lot less points because some points were stacked on top of each other because professors had similar values or the scatterplot affixes points to nearest value. The jitter adds some noise, which spreads each observation out a bit so there's less or zero chance of stacking.**
...
**Linearity plot: There doesn't seem to be any pattern, but there are some negative outliers. Histogram plot: left skewed. QQ Plot: left skew. Overall, the residual plot shows slight concern, but it's not so bad as to dismiss the model.**
...
**Yeah, the conditions look mostly reasonable. The residual plot does not show a clear pattern, so linearity seems fine. The histogram and QQ plot are not perfect and show some left skew, but it is not bad enough to throw out the model. Overall, the model seems okay to interpret.**
...
**Yeah, after adding gender to the model, it increased from 0.06664 to 0.07416**
...
**You would just keep hatbeta_2 since it's multiplied by 1 and we can see from the plot that color is lower than b&w, so b&w will have a higher course evaluation score.**
...
**It creates 2 dummy variables based off rank (teaching is built into the model), which are ranktenure track and ranktenured. **
...
**Close, but it seems that cls_profsingle had the highest p-value.**
...
**Since ethinicity has two variables, not_minority and minority (built in) if we keep everything else the same, not minority have evaluations 0.123 higher than minority professors.**
...
**No, the coefficients and significance did not change much. This means that cls_prof was not strongly colinear with the other variables.**

```{r}
m_semi <- lm(score ~ rank + gender + ethnicity + language + age + cls_perc_eval 
...
**I took out the highest var with p value above 0.05 until I got the following: predicted score = 3.771922 + 0.207112(gendermale) + 0.167872(ethnicitynot minority) - 0.206178(languagenon-English) - 0.006046(age) + 0.004656(cls_perc_eval) + 0.505306(cls_creditsone credit) + 0.051069(bty_avg) - 0.190579(pic_colorcolor)**

```{r}
m_backwards <- lm(score ~ gender + ethnicity + language + age + cls_perc_eval + cls_credits + bty_avg + pic_color, data = evals)
summary(m_backwards)
```
...
**It looks fine, not much different from m_bty, the regression conditions are reasonable.**

```{r}
ggplot(data = m_backwards, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("Fitted values") +
  ylab("Residuals")

ggplot(data = m_backwards, aes(x = .resid)) +
  geom_histogram(bins = 25) +
  xlab("Residuals")

ggplot(data = m_backwards, aes(sample = .resid)) +
  stat_qq()
```
...
**Yeah, if each observation is a class and professors can show up multiple times they affect the independence condition.**

18. Based on your final model, describe the characteristics of a professor and 
    course at University of Texas at Austin that would be associated with a high
    evaluation score.

**Male, not minority, English-speaking, younger, higher beauty rating, higher percent of students completing evaluations, one-credit course, and black-and-white photo.**

19. Would you be comfortable generalizing your conclusions to apply to professors
    generally (at any university)? Why or why not?

**No, this data came from one university. If it was a randomized sample of multiple universities, then sure.**
