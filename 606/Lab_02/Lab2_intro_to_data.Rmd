...
#### Answer 1.
Yes. The data is skewed between 0-500, so when binwidth is high
(examples 1 & 3) it creates the illusion that more flights have a
negative/0 delay. However, when binwidth is low, like example 2 with 15,
you can see that there are more flights with \> 0 delay. This is a good
example of when granularity is important.
...
```{r}
sfo_feb_flights |> ggplot(aes(x = arr_delay)) + geom_histogram(binwidth = 20)
# median fits better since mean would be skewed
sfo_feb_flights |> summarize(median_arr = median(arr_delay, na.rm = 0),
                             iqr_arr = IQR(arr_delay, na.rm = 0),
                             min_arr = min(arr_delay, na.rm = 0), 
                             max_arr = max(arr_delay, na.rm = 0))
```
I decided to go with four (median, iqr, min, max). Because of the skewed distribution I think the median reflects the data the best. IQR measures width of the middle distribution, while the min/max provides context for how widely distributed this data is.
...
**Insert your answer here**
```{r}
sfo_feb_flights |> summarize(arr_median = median(arr_delay), arr_irq = IQR(arr_delay), .by = carrier)
```
It's a tie between DL and UA with an iqr of 22. 
...
Similar. They both have the threat of being an outlier in the dataset. Mean avg can be 0, but 49% of the data can be extremely delayed. Same with median, the flight delays are typically 0, but how extreme is the outlier? That's why IQR is useful. 
...
LGA. It has the highest on time departure rate.  
...
**Insert your answer here**
```{r}
df <- nycflights |> mutate(avg_speed = distance / (air_time / 60)) 
str(df)
```
...
**Insert your answer here**
```{r}
df |> ggplot(aes(y = avg_speed, x = distance)) + geom_point() + geom_smooth()
```
Average speed increases as distance increases. 
