...
**Scatter. Personal Freedom Score and PF Expression Control looks linear and using a linear regression to predict personal freedom score seems to be ok.**

```{r}
hfi |> select(pf_score, pf_expression_control) |>
...
**The relationship is positive and linear, There's a couple of outliers but the strength of the relationship is linear.**
...
**My initial was 1554.82, the next attempt was 987.066, which was the best out of 3 more attempts.**
...
\] 
**The slope tells us that the relationship is positive.**
...
4.61707 + 0.49143 * 6.7
```
**7.909651. It's likely an underestimate since it seems the data in that region is mostly above the line. According to the m1 summary the residual standard error would be 0.8318 off, so we can use that number.**
...
**Nope, seems to be scattered around 0 pretty evenly. There's a bit more negative outliers but not that much. This suggests that the pattern is mainly linear between the two variables.**
...
**Yes, the histogram looks fairly normal and outside of some uneven tails, looks fairly normal.**
...
**Positive and non-linear more than linear.**
```{r}
hfi |>
  ggplot(aes(x = pf_rol_criminal , y = pf_score)) +
  geom_point() +
  stat_smooth(method = "lm", se = FALSE)
```
...
**m1 R^2 = 06342, m3 R^2 = 0.4656, which means that pf_expression control is a better predictor since it accounts for a higher amount of variability in pf_score**

```{r}
m3 <- lm(pf_score ~ pf_rol_criminal, data = hfi)
summary(m1)
summary(m3)
```
...
**pf_expression_killed had a R^2 value of 0.05902, which is 5.9% of the variability in determining pf_score. Really weak, kinda surprised. But when you take a look at the data, it's all over the place with lots of clustering around 0 (journalists get killed) and 10 (journalist don't get killed), so you can see why it's not a great indicator.**
```{r}
m4 <- lm(pf_score ~ pf_expression_killed, data = hfi)
summary(m4)

hfi |>
  ggplot(aes(x = pf_expression_killed , y = pf_score)) +
  geom_point() +
  stat_smooth(method = "lm", se = FALSE)
