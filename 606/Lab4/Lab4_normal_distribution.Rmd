...
fastfood |>
  ...
  ggplot(aes(x = restaurant, y = cal_fat, fill = restaurant)) +
  geom_point()
...
**McDonald's has more cal_fat than Dairy Queen, which is evident from the column. From the point we can see that McDonald's is mainly within the 0-500 range with some outliers to 1500 cal_fat.It also looks like McDonald's has more data points than Dairy Queen. If we take a look at the histogram Dairy has a somewhat normal distribution, slightly skewed right, while McDonald's is heavily skewed right.**
...
**It's slightly skewed right, otherwise it appears it would follow a normal distribution.**
...
**Yes, all points fall on the line. I would say that the sim_norm version follows more diagonally than the real data, but they aren't that different.**
...
**Similar to question 3, I said that they are pretty similar, but the simulated data has more of a normal distribution than the actual data.**
...
**Big difference. The simulated data has a nice, nearly, diagonal line, while the actual data is heavily skewed towards 0, almost looks logarithmic.**
...
1 - pnorm(q = 600, mean = cfmean, sd = cfsd)

chick_fil_a |>
  filter(cal_fat > 600) |>
  summarize(percent = n() / nrow(chick_fil_a))

cfmean <- mean(chick_fil_a$protein)
cfsd   <- sd(chick_fil_a$protein)

1 - pnorm(q = 40, mean = cfmean, sd = cfsd)
...
**I viewed the probability of `Chick Fil-A` items having more than 600 cal_fat, also the probability of their items having more than 40g of protein. The cal_fat data ~ theoretical: 4.46471e-06 / empirical: 0 | protein data ~  theoretical: 0.312024 / empirical: 0.111. The cal_fat had a closer agreement because both were essentially 0, way closer than the protein comparison.* 
**
...
**More samples at the same levels of sodium cause the step patterns. The restaurant's case could be be that the fast food restaurant uses items with similar sodium counts or maybe they round their values**
...
**For Chick Fil-A, total carbs for the probability plot and histogram show a right skew.**
