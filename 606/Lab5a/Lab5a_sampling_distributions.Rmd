---
startLab("Lab5a")title: "Foundations for statistical inference - Sampling distributions"
author: ""
output:
...
**I did not get the same. The sample distribution was Benefits n = 43 and p = 0.86, so it sampled more counts from Benefits than the population.**
...
**No, unless they chose a different seed. However, I don't expect the difference to be that much, as stated earlier the difference should be less than 1% since we are sampling from the same population set.**
...
**It actually differs slightly, closer to the actual representation of the total population (off by 0.02) and compared to samp1 (off by 0.04). You would expect that the more samples, the closer representation to the total population, however in my case this was not true as samp100 had a 1:1 comparison vs samp1000, which was off by 0.004 - which isn't too far off but it's still further away than samp100 so maybe it's just noise.**
...
**1500 elements, since we have 1500 reps. We can calculate the center by just doing a mean of the p_hat column for sample_props50, which is 0.20003947. Looks like a normal distribution, maybe right skewed a little.**
...
**There are 20 elements, but that's due to the filtering. If I didn't filter there would be 25. Each element is a sample proportion derived from a replicate sample**
...
**Each observation represents a sample proportion from a simulated sample. The mean remains very similar, however the standard error decreases as you go larger in size. It seems the mean distribution for 10 is strange until it reaches a sample size of 20, at which the mean distribution stays pretty consistently as 0.2**
...
samp15 |> count(scientist_work) |>
  mutate(p = n / sum(n))

```

**I get p = 0.8, so 12/15 people believe that scientist work enchances their lives.**
...
**The distribution is left skewed. From this data you can assume that the true proportion believe work scientists do enchances their lives, the number I would guess is 80% since it has the most counts and the distribution seems to be centered around it.**
...
**sample_props150 distribution looks like a normal distribution rather than sample_props15 which was left skewed. Also, the counts for p = 0.8 are much higher comparable to the other p values in this distribution plot. **
...
**Between sample size of 15 and 150, 150 would have the smaller spread. The reason is because the SE decreases as the sampling increases. If you want to be close to the true value, you would prefere a sampling distribution with a small spread. **
