...
**It should be 60 and from my code the p value = 0.617, so nearly 61.7%, which is nearly 62%.**

2.  Would you expect another student's sample proportion to be identical to yours? 
    Would you expect it to be similar? Why or why not?

**If they used the same seed, it should be the same, otherwise it would be different. However, the difference should be quite small since we are sampling from the same population.**
...
**This means that if we took random samples in the same way and built a confidence for each one, about 95% of those samples capture the true population proportion.**
...
**My confidence interval is 0.5 - 0.734. My neighbor's interval should capture the similar values, but not the same unless they are using the same seeds**
...
**95%, because the confidence level is 95%**
...
```{r shiny, eval=FALSE, include=FALSE, results=TRUE}
...
**Not exactly 95% because 50 intervals is still a relatively small number, so random variation is expected. If we increased the reps, the proportion would tend to get closer to 95%.**
...
**I chose a 90% confidence level. I would expect a 90% confidence interval to be narrower than a 95% confidence interval because lower confidence requires less uncertainty to be captured.**
...
**The 90% confidence interval is 0.5 - 0.717**
...
**It's pretty close, however it's not exact**
...
**I chose 99%. I expected it to be wider than the others because higher confidence means a bigger range. Most of the intervals still included the true proportion, which is what you’d expect with a higher confidence level.**
...
**As sample size increases, the intervals get narrower. Smaller samples make the intervals wider because there’s more uncertainty**
...
**Increasing bootstrap samples doesn’t really change the width. It just makes the result more consistent. The standard error mostly depends on the sample size.**
